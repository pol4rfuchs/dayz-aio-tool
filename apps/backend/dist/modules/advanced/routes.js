...
function classify(name, category = "") {
    ...
    if (/food|can|meat|fruit|drink|soda|water/.test(s))
        return "food";
    if (/weapon|rifle|pistol|shotgun|ak|m4|mosin|sks|ammo|magazine/.test(s))
        return "weapons";
    if (/medical|bandage|saline|blood|morphine|epinephrine|charcoal|vitamin/.test(s))
        return "medical";
    if (/tool|knife|axe|hammer|shovel|wrench|saw/.test(s))
        return "tools";
    return "other";
}
async function readTypes(serverId) {
...
export async function advancedRoutes(app) {
    ...
    app.post("/api/servers/:serverId/dynamic-economy/apply", async (request, reply) => {
        try {
            const { serverId } = request.params;
            const input = dynamicRuleSchema.parse(request.body);
            if (input.confirm !== "APPLY_DYNAMIC_ECONOMY_TO_TEST_COPY")
                return reply.code(400).send({ error: "Apply blocked. Set confirm to APPLY_DYNAMIC_ECONOMY_TO_TEST_COPY after testing on a copied server." });
    ...
    app.get("/api/servers/:serverId/map-tools", async (request, reply) => {
        try {
            ...
            let points = [];
            try {
                points = parseEventSpawns(await fs.readFile(filePath, "utf8"));
            }
            catch { /* optional */ }
    ...
    app.post("/api/servers/:serverId/ai/analyze", async (request, reply) => {
        try {
            ...
            let economy = { valid: false, errors: ["types.xml not checked"], warnings: [], count: 0 };
            try {
                economy = validateTypesXml(await fs.readFile(path.join(server.missionPath, "db", "types.xml"), "utf8"));
            }
            catch (e) {
                economy.errors = [e.message];
            }
            const findings = [];
            if (!status.pidAlive)
                findings.push({ severity: "info", title: "Server process is not running", detail: "Start/Stop tests should run only after Readiness passes." });
            if (!economy.valid)
                findings.push({ severity: "warn", title: "Economy validation failed", detail: formatValidationSummary(economy) });
            else if (economy.warnings?.length)
                findings.push({ severity: "warn", title: "Economy validation warnings", detail: formatValidationSummary(economy) });
            if (logs.some((line) => /error|exception|crash|fatal/i.test(line)))
                findings.push({ severity: "warn", title: "Runtime logs contain error-like lines", detail: logs.filter((line) => /error|exception|crash|fatal/i.test(line)).slice(-10).join("\n") });
            if (backups === 0)
                findings.push({ severity: "warn", title: "No backups recorded", detail: "Run a manual backup before editing config/economy files." });
            if (!findings.length)
                findings.push({ severity: "ok", title: "No obvious issue detected", detail: "Doctor/Readiness/Safety tests still remain the source of truth." });
