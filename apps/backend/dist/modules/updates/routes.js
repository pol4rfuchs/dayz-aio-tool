...
function resolveSteamLogin(input) {
...
function steamFailureReason(exitCode, analysis) {
    if (exitCode !== 0)
...
async function getServerExeSnapshot(server) {
...
async function readWindowsExeVersion(exePath) {
...
function serverExeChanged(before, after) {
...
async function readAppManifestSummary(server) {
...
function enqueueUpdateJob(job, runner) {
    rememberJob(job);
...
function resolveSteamCmdPath(server) {
    if (server.steamcmdPath && String(server.steamcmdPath).trim())
        return String(server.steamcmdPath).trim();
    return path.join(path.dirname(server.rootPath), "SteamCMD", "steamcmd.exe");
}
function workshopStagingRoot(server) {
    return path.join(path.dirname(server.rootPath), "Workshop");
}
function parseLaunchParamValues(params, name) {
...
function collectModIdsFromLaunchAndDb(server) {
    const fromLaunch = [
        ...parseLaunchParamValues(server.launchParams ?? "", "mod"),
        ...parseLaunchParamValues(server.launchParams ?? "", "serverMod")
    ];
    const fromDb = getDb().prepare("SELECT workshop_id as workshopId, folder_name as folderName FROM mods WHERE server_id = ? AND enabled = 1 ORDER BY load_order ASC").all(server.id)
        .flatMap((row) => [row.workshopId, /^\d{6,}$/.test(row.folderName) ? row.folderName : ""])
        .filter((value) => Boolean(value && /^\d{6,}$/.test(value)));
    return uniq([...fromLaunch, ...fromDb]);
}
async function copyWorkshopItemToServer(server, workshopId) {
...
async function dirStatSummary(root) {
    try {
        const stat = await fs.stat(root);
        let fileCount = 0;
        let totalBytes = 0;
        async function walk(current, depth = 0) {
            if (depth > 5 || fileCount > 20_000)
                return;
            let entries = [];
            try {
...
async function listDirectFiles(root, matcher) {
    try {
...
async function listRecursiveBasenames(root, matcher, limit = 120) {
    const out = [];
    async function walk(current, depth = 0) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory())
                ...
                out.push(path.basename(full));
        }
    }
    await walk(root);
    return [...new Set(out)].sort();
}
async function readModDisplayName(modPath) {
...
async function workshopSyncReport(server) {
  ...
  `).all(server.id);
    ...
    for (const workshopId of ids) {
        ...
        const flags = [];
        if (!serverStat.exists)
            flags.push("missing_server_folder");
        if (!stagingStat.exists)
            flags.push("missing_staging_download");
        if (stagingStat.exists && serverStat.exists && Number(stagingStat.mtimeMs || 0) > Number(serverStat.mtimeMs || 0) + 1000)
            flags.push("staging_newer_than_server");
        if (!keyNames.length)
            flags.push("mod_has_no_bikey");
        if (keyNames.length && copiedKeys.length < keyNames.length)
            flags.push("server_keys_not_synced");
        if ([folderName, row?.displayName || "", modName, ...pboHints].join(" ").toLowerCase().includes("expansion"))
            flags.push("expansion_related");
...
async function updaterPreflight(server) {
...
export async function updateRoutes(app) {
    ...
    app.get("/api/updates/jobs/:jobId", async (request, reply) => {
        ...
        if (!job)
            return reply.code(404).send({ error: "Update job not found" });
        return job;
    });
    ...
    app.post("/api/servers/:serverId/updates/workshop-sync-from-staging", async (request, reply) => {
        try {
            ...
            if (preflight.checks.some((check) => check.name === "server_stopped" && check.status === "fail"))
                return reply.code(400).send({ error: "Stop the DayZ server before syncing Workshop folders from staging.", preflight });
            const modIds = preflight.modIds;
            const results = [];
            for (const workshopId of modIds) {
                const copied = await copyWorkshopItemToServer(server, workshopId);
                if (copied)
                    registerNumericWorkshopMod(serverId, workshopId);
    ...
    app.post("/api/servers/:serverId/updates/server", async (request, reply) => {
        try {
            ...
            const preflight = await updaterPreflight(server);
            if (preflight.checks.some((check) => check.name === "server_stopped" && check.status === "fail"))
                return reply.code(400).send({ error: "Stop the DayZ server before updating server binaries.", preflight });
            if (!await exists(preflight.steamcmdPath))
                return reply.code(400).send({ error: `SteamCMD not found: ${preflight.steamcmdPath}`, preflight });
            const job = { id: crypto.randomUUID(), serverId, action: "server-update", status: "queued", total: 1, completed: 0, failed: 0, results: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    ...
    app.post("/api/servers/:serverId/updates/mods", async (request, reply) => {
        try {
            ...
            const preflight = await updaterPreflight(server);
            if (preflight.checks.some((check) => check.name === "server_stopped" && check.status === "fail"))
                return reply.code(400).send({ error: "Stop the DayZ server before updating Workshop mods.", preflight });
            if (!await exists(preflight.steamcmdPath))
                return reply.code(400).send({ error: `SteamCMD not found: ${preflight.steamcmdPath}`, preflight });
            const modIds = preflight.modIds;
            if (!modIds.length)
                return reply.code(400).send({ error: "No Workshop IDs found. Run Launch Profile Import or Mods scan first.", preflight });
            await fs.mkdir(workshopStagingRoot(server), { recursive: true });
            const job = { id: crypto.randomUUID(), serverId, action: "mods-update", status: "queued", total: modIds.length, completed: 0, failed: 0, results: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
            enqueueUpdateJob(job, async () => {
                for (const workshopId of modIds) {
