...
import { parseEventsXml, parseGlobalsXml, parseSpawnableTypesSummary, parseTypesXml, updateEventsXmlFromItems, updateGlobalsXmlFromItems, updateTypesXmlFromItems, validateEconomyXml, validateTypesXml, assertEconomyXmlSize } from "./parser.js";
...
const saveXmlSchema = z.object({
    xml: z.string().min(1).refine((value) => {
        try {
            assertEconomyXmlSize(value, "submitted economy XML");
            return true;
        }
        catch {
            return false;
...
function getMissionPath(serverId) {
...
function getEconomyPath(serverId, file = "types.xml") {
    if (!ECONOMY_FILES.has(file))
        throw Object.assign(new Error(`Unsupported economy file: ${file}`), { statusCode: 400 });
    const server = requireServer(serverId);
...
function parseByFile(file, xml) {
    if (file === "types.xml")
        return { items: parseTypesXml(xml) };
    if (file === "events.xml")
        return { events: parseEventsXml(xml) };
    if (file === "globals.xml")
        return { globals: parseGlobalsXml(xml) };
    if (file === "cfgspawnabletypes.xml")
        return { spawnable: parseSpawnableTypesSummary(xml) };
    return {};
}
export async function economyRoutes(app) {
    ...
    app.put("/types/items", async (request, reply) => {
        try {
            const { serverId } = request.params;
            const input = saveTypesItemsSchema.parse(request.body);
            const filePath = getEconomyPath(serverId, "types.xml");
    ...
    app.put("/events/items", async (request, reply) => {
        try {
            const { serverId } = request.params;
            const input = saveEventsItemsSchema.parse(request.body);
    ...
    app.put("/globals/items", async (request, reply) => {
        try {
            const { serverId } = request.params;
            const input = saveGlobalsItemsSchema.parse(request.body);
    ...
    app.get("/:file", async (request, reply) => {
        try {
            const { serverId, file } = request.params;
            const filePath = getEconomyPath(serverId, file);
            const xml = await fs.readFile(filePath, "utf8");
