...
export function createStoredZip(entries) {
    ...
    for (const entry of entries) {
        ...
        if (content.length > 0xffffffff)
            throw new Error(`ZIP entry is too large for non-Zip64 writer: ${name}`);
        if (offset > 0xffffffff)
            throw new Error("ZIP archive offset exceeds non-Zip64 limit.");
    ...
    if (entryCount > 65535)
        throw new Error(`ZIP entry limit exceeded after filtering: ${entryCount} > 65535.`);
    if (centralStart > 0xffffffff || centralSize > 0xffffffff)
        throw new Error("ZIP archive exceeds non-Zip64 size limits.");
