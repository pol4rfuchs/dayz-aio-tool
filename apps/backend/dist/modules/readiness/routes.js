...
export async function readinessRoutes(app) {
    app.get("/api/system/readiness", async () => {
        ...
        try {
            ...
            checks.push({ key: "sqlite", label: "SQLite access", status: "fail", message: error.message });
        }
    ...
    app.get("/api/servers/:serverId/readiness", async (request, reply) => {
        try {
            ...
            try {
                ...
                checks.push({ key: "config.serverdz.read", label: "serverDZ.cfg readable", status: "fail", message: error.message, path: serverCfg, requiredFor: ["config-save", "start"] });
            }
            if (typesXml && await exists(typesXml)) {
                try {
                    ...
                    checks.push({ key: "economy.types.read", label: "types.xml readable", status: "fail", message: error.message, path: typesXml, requiredFor: ["economy-save"] });
                }
            }
            else {
