...
async function safeStat(file) { try {
    return await fs.lstat(file);
}
catch {
    return null;
} }
async function collectLogFiles(root, max = 500) {
    const files = [];
    ...
    async function walk(current) {
        ...
        for (const entry of entries) {
            ...
            if (!stat || stat.isSymbolicLink())
                continue;
            try {
                assertInsideRoot(rootSafe, full);
            }
            catch {
                continue;
            }
            if (stat.isDirectory()) {
                if (/node_modules|steamapps|workshop|backup|backups/i.test(entry.name))
                    continue;
                await walk(full);
            }
...
async function tail(file, maxBytes) {
    const stat = await fs.lstat(file);
    if (stat.isSymbolicLink() || !stat.isFile())
        throw Object.assign(new Error("Refusing to read non-file log target."), { statusCode: 400 });
...
export async function logRoutes(app) {
    ...
    app.get("/api/servers/:serverId/live-logs/file", async (request, reply) => {
        try {
            ...
            const bytes = Math.min(Math.max(Number(request.query.bytes ?? 24000), 1000), 250000);
            const server = requireServer(serverId);
