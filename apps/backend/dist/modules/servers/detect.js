...
function normalizeWindowsLikePath(input) {
    return input.trim().replace(/^"|"$/g, "");
}
async function readMissionTemplate(configPath) {
    try {
...
async function findMissionPath(rootPath, explicitMissionPath, missionTemplate) {
    if (explicitMissionPath && await isDirectory(explicitMissionPath))
        return explicitMissionPath;
    const missionsRoot = path.join(rootPath, "mpmissions");
    if (missionTemplate) {
        const exact = path.join(missionsRoot, missionTemplate);
        if (await exists(path.join(exact, "db", "types.xml")))
            return exact;
    }
    try {
        ...
        for (const candidate of candidates) {
            if (missionTemplate && path.basename(candidate).toLowerCase() === missionTemplate.toLowerCase() && await exists(path.join(candidate, "db", "types.xml")))
                return candidate;
        }
        for (const candidate of candidates) {
            if (await exists(path.join(candidate, "db", "types.xml")))
                return candidate;
        }
    }
    catch {
...
export async function detectExistingServer(rawInput) {
