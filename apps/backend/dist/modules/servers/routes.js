...
export async function serverRoutes(app) {
    ...
    app.post("/detect", async (request, reply) => {
        try {
            return await detectExistingServer(request.body);
        }
        catch (error) {
    ...
    app.post("/launch-profile/detect", async (request, reply) => {
        try {
            return await detectLaunchProfile(request.body);
        }
        catch (error) {
            return sendError(reply, error);
        }
    ...
    app.post("/", async (request, reply) => {
        try {
            ...
            if (!detected.valid)
                return reply.code(400).send({ error: "Server path validation failed", detection: detected });
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(id, input.name, detected.rootPath, detected.profilePath, detected.executablePath, detected.missionPath, input.launchParams || detected.launchParams, input.rconHost ?? null, input.rconPort ?? null, encryptSecret(input.rconPassword ?? null), input.steamcmdPath ?? null, input.workshopAppId ?? "221100", now, now);
    ...
    app.get("/:id/control", async (request, reply) => {
        try {
            return await getServerControlSummary(request.params.id);
        }
        catch (error) {
            return sendError(reply, error);
        }
    ...
    app.get("/:id/start/preflight", async (request, reply) => {
        try {
            return await preflightStartServer(request.params.id);
        }
        catch (error) {
            return sendError(reply, error);
        }
    ...
    app.post("/:id/start", async (request, reply) => {
        try {
            return await startServer(request.params.id);
        }
        catch (error) {
            return sendError(reply, error);
        }
    ...
    app.post("/:id/stop", async (request, reply) => {
        try {
            return await stopServer(request.params.id);
        }
        catch (error) {
            return sendError(reply, error);
        }
    ...
    app.post("/:id/restart", async (request, reply) => {
        try {
            return await restartServer(request.params.id);
        }
        catch (error) {
            return sendError(reply, error);
        }
    });
    app.get("/:id/status", async (request) => getRuntimeStatus(request.params.id));
