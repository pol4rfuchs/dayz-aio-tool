...
async function workshopPreflight(server) {
    const checks = [];
    if (!server.steamcmdPath)
        checks.push({ name: "steamcmd_path", status: "fail", message: "steamcmdPath is not configured on this server." });
    else {
        try {
            await fs.access(server.steamcmdPath);
            checks.push({ name: "steamcmd_path", status: "pass", message: server.steamcmdPath });
        }
        catch {
            checks.push({ name: "steamcmd_path", status: "fail", message: `SteamCMD not found: ${server.steamcmdPath}` });
        }
    }
    try {
        await fs.access(server.rootPath);
        checks.push({ name: "root_path", status: "pass", message: server.rootPath });
    }
    catch {
        checks.push({ name: "root_path", status: "fail", message: `Server root missing: ${server.rootPath}` });
    }
    ...
    try {
        await fs.access(steamapps);
        checks.push({ name: "steamapps", status: "pass", message: steamapps });
    }
    catch {
        checks.push({ name: "steamapps", status: "warn", message: `steamapps folder not found yet. SteamCMD may create it: ${steamapps}` });
    }
    return { ok: !checks.some((c) => c.status === "fail"), checks };
}
function enqueueWorkshopJob(job, runner) {
    rememberJob(job);
...
async function registerInstalledMod(serverId, workshopId, folderName) {
...
export async function workshopRoutes(app) {
    ...
    app.get("/api/workshop/jobs/:jobId", async (request, reply) => {
        ...
        if (!job)
            return reply.code(404).send({ error: "Workshop job not found" });
        return job;
    });
    ...
    app.post("/api/servers/:serverId/workshop/install", async (request, reply) => {
        try {
            ...
            const job = { id: crypto.randomUUID(), serverId, action: "install", status: "queued", total: 1, completed: 0, failed: 0, results: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
            enqueueWorkshopJob(job, async () => {
                ...
                writeAudit({ serverId, action: ok ? "workshop.install" : "workshop.install.failed", target: input.workshopId, metadata: { jobId: job.id, exitCode: result.exitCode, outputTail: result.output.slice(-4000) } });
                broadcast("workshop.install.finished", { workshopId: input.workshopId, exitCode: result.exitCode, jobId: job.id }, serverId);
                if (ok)
                    await registerInstalledMod(serverId, input.workshopId, folderName);
            });
    ...
    app.post("/api/servers/:serverId/workshop/update-enabled", async (request, reply) => {
        try {
            const { serverId } = request.params;
            const input = updateEnabledSchema.parse(request.body) || { username: "anonymous" };
            const server = requireServer(serverId);
            const rows = getDb().prepare("SELECT workshop_id as workshopId, folder_name as folderName FROM mods WHERE server_id = ? AND enabled = 1 AND workshop_id IS NOT NULL AND workshop_id != '' ORDER BY load_order ASC").all(serverId);
            const job = { id: crypto.randomUUID(), serverId, action: "update-enabled", status: "queued", total: rows.length, completed: 0, failed: 0, results: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
            enqueueWorkshopJob(job, async () => {
                for (const row of rows) {
                    ...
                    const result = await runSteamCmd(server, row.workshopId, input.username || "anonymous");
                    const failed = result.exitCode !== 0 ? 1 : 0;
                    job.results.push({ workshopId: row.workshopId, folderName: row.folderName, exitCode: result.exitCode, outputTail: result.output.slice(-2000) });
