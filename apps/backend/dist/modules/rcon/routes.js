...
async function sendUdpProbe(host, port, payload, timeoutMs = 2500) {
...
function rconConfig(serverId) {
    const server = requireServer(serverId);
    const password = decryptSecret(server.rconPassword);
    if (!server.rconHost || !server.rconPort || !password)
        throw Object.assign(new Error("RCON is not configured for this server."), { statusCode: 400 });
    return { server, host: server.rconHost, port: Number(server.rconPort), password };
}
async function runRcon(serverId, command) {
    if (!ALLOWLIST.test(command.trim()))
        throw Object.assign(new Error("Command blocked by RCON allowlist."), { statusCode: 400 });
...
export async function rconRoutes(app) {
