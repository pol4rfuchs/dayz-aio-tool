...
export function rotateFileIfNeededSync(filePath, maxBytes, maxFiles) {
    try {
...
let flushTimer = null;
let flushing = false;
function sanitizeForBackendLog(value) {
    return value
...
export function appendBackendLog(event, payload = {}) {
    try {
...
export function registerBackendFileLogging(app) {
    ensureRuntimeLogDirsSync();
...
export async function writeJsonSnapshot(name, payload) {
