...
export async function notifyAll(message: string, title?: string) {
  const targets = getDb().prepare("SELECT id, name, type, url, topic FROM notification_targets WHERE enabled = 1").all() as any[];
  const results = [];
  for (const target of targets) {
    try { results.push({ target: target.name, ...(await postNotification(target, message, title)) }); }
    catch (error) { results.push({ target: target.name, ok: false, error: String(error) }); }
  }
  return results;
}
...
export async function notificationRoutes(app: FastifyInstance) {
  ...
  app.post("/api/notifications/:id/test", async (request, reply) => {
    try {
      ...
      const target = getDb().prepare("SELECT id, name, type, url, topic FROM notification_targets WHERE id = ?").get(id) as any;
      if (!target) return reply.code(404).send({ error: "Notification target not found" });
