...
function steamArgs(rootPath: string, workshopAppId: string, workshopId: string, username = "anonymous") {
  return ["+force_install_dir", rootPath, "+login", username || "anonymous", "+workshop_download_item", workshopAppId, workshopId, "+quit"];
}

function redactSteamArgs(args: string[]) {
  return args.map((arg, index) => index > 0 && args[index - 1] === "+login" && arg !== "anonymous" ? "<steam-user>" : arg);
}

async function runSteamCmd(server: any, workshopId: string, username = "anonymous") {
