...
async function readTextForBundle(filePath: string) {
...
async function addFile(entries: ZipEntry[], source: string, zipName: string, options?: { mask?: boolean; optional?: boolean }) {
  try {
    ...
    if (!options?.optional) {
      entries.push({ name: `${zipName}.missing.txt`, content: `Could not read ${source}: ${(error as Error).message}\n` });
    }
  }
}

async function addFilesFromDir(entries: ZipEntry[], dir: string, zipPrefix: string, pattern?: RegExp) {
  try {
...
async function findRepoRoot() {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (parsed.name === "dayz-aio" || parsed.workspaces) return candidate;
    } catch {
      // try next candidate
    }
  }
...
async function readProjectVersion(repoRoot: string) {
  try {
