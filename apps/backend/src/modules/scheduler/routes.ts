...
function computeNextRun(input: { intervalMinutes?: number | null; atTime?: string | null }, from = new Date()) {
  if (input.intervalMinutes) return new Date(from.getTime() + input.intervalMinutes * 60_000).toISOString();
  if (input.atTime) {
...
export async function runScheduleAction(serverId: string, action: string) {
  if (action === "start") return startServer(serverId);
  if (action === "stop") return stopServer(serverId);
  if (action === "restart") return restartServer(serverId);
  if (action === "backup") return createManualBackup(serverId);
...
export async function tickSchedules() {
  if (scheduleTickRunning) return 0;
  ...
  try {
    ...
    `).all(now.toISOString()) as Array<{ id: string; serverId: string; name: string; action: string; intervalMinutes?: number | null; atTime?: string | null }>;
    ...
    for (const item of due) {
      try {
        ...
        `).get(message, new Date().toISOString(), item.id) as { failureCount: number } | undefined;
...
export async function schedulerRoutes(app: FastifyInstance) {
  ...
  app.post("/api/schedules", async (request, reply) => {
    try {
      ...
      if (!input.intervalMinutes && !input.atTime) return reply.code(400).send({ error: "intervalMinutes or atTime is required" });
