...
function modRows(serverId: string) {
...
function normalizeModIdentity(name: string) {
...
async function listFilesRecursive(root: string, matcher: (name: string) => boolean, limit = 5000) {
  const result: string[] = [];
  async function walk(current: string) {
    ...
    for (const entry of entries) {
      ...
      else if (entry.isFile() && matcher(entry.name)) result.push(full);
    }
  }
  await walk(root);
  return result;
...
async function readSmallTextIfExists(file: string, maxBytes = 128_000) {
  try {
...
function parseArrayAssignments(text: string, names: string[]) {
...
async function analyzeMods(serverId: string) {
  const server = requireServer(serverId);
  const mods = modRows(serverId) as Array<{ folderName: string; displayName?: string; workshopId?: string; enabled: number; loadOrder: number; hasKeys: number }>;
  const issues: ModDiagnosticIssue[] = [];
  const normalized = new Map<string, string[]>();
  const pboOwners = new Map<string, string[]>();
  const keyOwners = new Map<string, string[]>();
  const declaredDependencies: Array<{ mod: string; dependencies: string[] }> = [];
  ...
  for (const mod of mods) {
    ...
    if (deps.length) declaredDependencies.push({ mod: folderName, dependencies: deps });
  }

  for (const [identity, owners] of normalized) {
    if (owners.length > 1) issues.push({ severity: "warning", code: "DUPLICATE_NORMALIZED_MOD_NAME", message: `Multiple mod folders normalize to the same identity: ${owners.join(", ")}`, details: { identity, owners } });
  }
  for (const [pbo, owners] of pboOwners) {
    const unique = [...new Set(owners)];
    if (unique.length > 1) issues.push({ severity: "warning", code: "DUPLICATE_PBO_BASENAME", message: `Same PBO basename appears in multiple mods: ${pbo}`, details: { pbo, owners: unique } });
  }
  for (const [key, owners] of keyOwners) {
    const unique = [...new Set(owners)];
    if (unique.length > 1) issues.push({ severity: "info", code: "DUPLICATE_BIKEY_NAME", message: `Same .bikey filename appears in multiple mods: ${key}`, details: { key, owners: unique } });
  }
  ...
  for (const entry of declaredDependencies) {
    ...
    if (missing.length) issues.push({ severity: "warning", code: "DECLARED_DEPENDENCY_NOT_MATCHED", mod: entry.mod, message: `Mod declares dependencies that were not matched against enabled folder names.`, details: { missing } });
  }
...
async function keySyncPlan(serverId: string) {
  const server = requireServer(serverId);
  const mods = modRows(serverId) as Array<{ folderName: string; enabled: number }>;
  const enabled = mods.filter((m) => m.enabled);
  const serverKeysPath = path.join(server.rootPath, "keys");
  const existingServerKeys = new Set<string>();
  try {
    const entries = await fs.readdir(serverKeysPath);
    for (const entry of entries.filter((name) => name.toLowerCase().endsWith(".bikey"))) existingServerKeys.add(entry.toLowerCase());
  } catch {
    // keys folder can be created by sync.
  }

  const plan: Array<{ mod: string; source: string; target: string; action: "copy" | "exists" }> = [];
  const missingModKeys: string[] = [];
  for (const mod of enabled) {
    const keyDir = path.join(server.rootPath, mod.folderName, "keys");
    let keyFiles: string[] = [];
    try { keyFiles = (await fs.readdir(keyDir)).filter((name) => name.toLowerCase().endsWith(".bikey")); }
    catch { keyFiles = []; }
    if (!keyFiles.length) {
...
async function syncKeys(serverId: string) {
  const server = requireServer(serverId);
  ...
  const copied: Array<{ source: string; target: string; mod: string }> = [];
...
export async function modRoutes(app: FastifyInstance) {
  ...
  app.get("/start-params", async (request, reply) => {
    try { return { modParam: buildModStartParams((request.params as { serverId: string }).serverId) }; }
    catch (error) { return sendError(reply, error); }
  });
}

export function buildModStartParams(serverId: string) {
  const mods = getDb().prepare("SELECT folder_name as folderName FROM mods WHERE server_id = ? AND enabled = 1 ORDER BY load_order ASC").all(serverId) as Array<{ folderName: string }>;
