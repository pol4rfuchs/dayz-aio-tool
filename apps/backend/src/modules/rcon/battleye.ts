...
function parsePacket(message: Buffer) {
  if (message.length < 8 || message.subarray(0, 2).toString("ascii") !== "BE") return null;
  const body = message.subarray(6);
  if (body[0] !== 0xff) return null;
  return body.subarray(1);
}

function onceMessage(socket: dgram.Socket, timeoutMs: number) {
  return new Promise<Buffer>((resolve, reject) => {
    ...
    function onMessage(message: Buffer) { cleanup(); resolve(message); }
...
async function sendAndWait(socket: dgram.Socket, host: string, port: number, payload: Buffer, timeoutMs: number) {
  const wait = onceMessage(socket, timeoutMs);
  await new Promise<void>((resolve, reject) => socket.send(packet(payload), port, host, (err) => err ? reject(err) : resolve()));
  return parsePacket(await wait);
}

export type BattleyeRconResult = {
  ok: boolean;
  command: string;
  response: string;
  packets: number;
...
async function sendBattleyeRconCommandRaw(opts: { host: string; port: number; password: string; command: string; timeoutMs: number }): Promise<BattleyeRconResult> {
  ...
  try {
    ...
    if (!first || first[0] !== 0x01) throw new Error("Unexpected BattlEye RCON command response.");

    const chunks: string[] = [];
    const firstText = first.subarray(2).toString("utf8");
    if (firstText) chunks.push(firstText);
    ...
    while (Date.now() < deadline) {
      try {
        ...
        if (!payload || payload[0] !== 0x01) continue;
        packets++;
...
let rconQueue: Promise<void> = Promise.resolve();

export function resetBattleyeRconQueueForTests() {
  rconQueue = Promise.resolve();
}

export function sendBattleyeRconCommand(opts: { host: string; port: number; password: string; command: string; timeoutMs: number }): Promise<BattleyeRconResult> {
