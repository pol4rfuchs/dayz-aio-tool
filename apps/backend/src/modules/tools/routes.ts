...
function calculateDayNight(input: z.infer<typeof dayNightSchema>) {
...
async function collectClassnames(serverId: string, query: string, limit: number) {
  ...
  const seen = new Map<string, { classname: string; source: string; nominal?: number; min?: number; category?: string }>();
  for (const file of files) {
    let xml = "";
    try { xml = await fs.readFile(file, "utf8"); } catch { continue; }
    if (path.basename(file).toLowerCase() === "types.xml") {
      for (const item of parseTypesXml(xml)) {
        if (!seen.has(item.name)) seen.set(item.name, { classname: item.name, source: "types.xml", nominal: item.nominal, min: item.min, category: item.category });
      }
    } else {
      for (const match of xml.matchAll(/<type\s+name="([^"]+)"/gi)) {
        const name = match[1];
        if (!seen.has(name)) seen.set(name, { classname: name, source: "cfgspawnabletypes.xml" });
      }
    }
  }
...
type SteamBanPayload = { players?: Array<Record<string, unknown>> };
...
export async function toolRoutes(app: FastifyInstance) {
  ...
  app.get("/api/servers/:serverId/economy/classnames", async (request, reply) => {
    try {
      ...
      const query = String((request.query as Record<string, unknown>).query ?? "");
      const limit = Math.min(Number((request.query as Record<string, unknown>).limit ?? 50), 250);
