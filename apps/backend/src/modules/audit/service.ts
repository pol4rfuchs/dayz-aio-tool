...
export function writeAudit(input: { serverId?: string; actor?: string; action: string; target: string; metadata?: unknown }) {
  const row = {
...
export function listAudit(serverId?: string, limit = 100) {
  ...
  return rows.map((row: any) => ({ ...row, metadata: row.metadata ? JSON.parse(row.metadata) : null }));
}
