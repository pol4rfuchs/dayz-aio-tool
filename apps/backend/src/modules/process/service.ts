...
const externalLogs = new Map<string, string[]>();
...
export async function preflightStartServer(serverId: string) {
...
function pushLog(serverId: string, line: string) {
  ...
  if (!proc) externalLogs.set(serverId, target);
...
export function getRuntimeStatus(serverId: string) {
  const proc = processes.get(serverId);
...
export function getAllRuntimeStatuses() {
  const rows = getDb().prepare("SELECT server_id as serverId FROM server_state").all() as Array<{ serverId: string }>;
  return rows.map((row) => getRuntimeStatus(row.serverId));
}

export function getLogs(serverId: string, limit = 300) {
  const proc = processes.get(serverId);
...
export async function startServer(serverId: string) {
  ...
  const record: RunningProcess = { child, startedAt, logs: [] };
  processes.set(serverId, record);

  (child as any).all?.on("data", (chunk: Buffer | string) => {
    const text = String(chunk);
    for (const line of text.split(/\r?\n/).filter(Boolean)) pushLog(serverId, line);
  });
...
function parseConfiguredPort(launchParams: string) {
...
async function getPortBinding(port: number) {
  try {
    ...
    return {
      ...
      checkedAt: new Date().toISOString(),
      lines: [],
      note: `Port check failed: ${(error as Error).message}`
    };
  }
}

export async function getServerControlSummary(serverId: string) {
  const server = requireServer(serverId);
