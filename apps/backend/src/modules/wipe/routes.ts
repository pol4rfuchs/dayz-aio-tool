...
async function dirExists(dir: string) { try { return (await fs.stat(dir)).isDirectory(); } catch { return false; } }

function missionRoot(server: { rootPath: string; missionPath?: string }) {
...
async function listStorages(missionPath: string) {
  let entries: import("node:fs").Dirent[] = [];
  try { entries = await fs.readdir(missionPath, { withFileTypes: true }); } catch { return []; }
  const result = [];
  for (const entry of entries) {
    if (!entry.isDirectory() || !/^storage_\d+$/i.test(entry.name)) continue;
...
export async function wipeRoutes(app: FastifyInstance) {
  ...
  app.post("/api/servers/:serverId/wipe/execute", async (request, reply) => {
    try {
      ...
      const server = requireServer(serverId);
      const runtime = getRuntimeStatus(serverId);
      if (runtime.pidAlive) return reply.code(400).send({ error: "Stop the DayZ server before executing a wipe." });
      const missionPath = missionRoot(server);
      const source = assertInsideRoot(server.rootPath, path.join(missionPath, input.storageName));
      if (!await dirExists(source)) return reply.code(404).send({ error: `Storage folder not found: ${source}` });
