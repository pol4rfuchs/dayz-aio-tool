...
export type LaunchProfileSource = {
  filePath: string;
  type: "json" | "script" | "generated";
  ...
  profilePath?: string;
  port?: number;
  importedFrom?: "launch-params" | "dayz-server-manager-json" | "dayz-server-manager-ps1" | "fallback";
};

export type LaunchProfileDetection = {
  rootPath: string;
  profilePath: string;
  recommendedLaunchParams: string;
  source?: LaunchProfileSource;
  sources: LaunchProfileSource[];
  modFolders: string[];
  modIdFolders: string[];
  managerMods: string[];
...
function normalizeParamText(input: string) {
  return input
    .replace(/`"/g, "\"")
...
function extractParamsFromText(text: string) {
  const matches = Array.from(text.matchAll(PARAM_RE)).map((match) => match[1]?.trim()).filter(Boolean) as string[];
  return normalizeParamText(matches.join(" "));
}

export function collectStrings(value: unknown, out: string[] = [], keyPath = "", depth = 0, seen = new WeakSet<object>()): string[] {
  ...
  if (typeof value === "string") {
    ...
    if (interestingKey || interestingValue) out.push(value);
    return out;
  }
  ...
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) collectStrings(value[index], out, `${keyPath}[${index}]`, depth + 1, seen);
    return out;
  }
  if (value && typeof value === "object") {
    for (const [key, nested] of Object.entries(value as Record<string, unknown>)) collectStrings(nested, out, keyPath ? `${keyPath}.${key}` : key, depth + 1, seen);
  }
  return out;
}

function pickBestJsonParamCandidate(parsed: unknown) {
  ...
  candidates.sort((a, b) => {
    const score = (input: string) => ["config", "profiles", "mod", "servermod", "bepath", "port"].filter((name) => hasParam(input, name)).length;
    return score(b) - score(a) || b.length - a.length;
  });
  return candidates[0] ?? "";
}

function extractArrayBlock(text: string, key: string) {
...
function extractManagerIdArray(text: string, key: string) {
...
function extractManagerStringArray(text: string, key: string) {
  const block = extractArrayBlock(text, key);
  if (!block) return [];
...
function extractJsonNumber(text: string, keys: string[], fallback: number) {
...
function extractJsonBoolean(text: string, key: string) {
  return new RegExp(`"${key}"\\s*:\\s*true`, "i").test(text);
}

function resolveMaybeRelative(rootPath: string, candidate: string) {
  const clean = normalize(candidate);
  if (!clean) return "";
  return path.isAbsolute(clean) ? clean : path.join(rootPath, clean);
}

function buildManagerLaunchSource(filePath: string, rootPath: string, detectedProfilePath: string, content: string): LaunchProfileSource | null {
  ...
  if (!mods.length && !serverMods.length && !profileSetting && !manualParams.length) return null;

  const params: string[] = [quoteParam("config", path.join(rootPath, "serverDZ.cfg"))];
  if (mods.length) params.push(quoteParam("mod", mods.join(";")));
  if (serverMods.length) params.push(quoteParam("serverMod", serverMods.join(";")));
  ...
  for (const [key, flag] of flagMap) if (extractJsonBoolean(content, key)) params.push(flag);

  const limitFps = extractJsonNumber(content, ["limitFps", "limitFPS"], 0);
  const cpuCount = extractJsonNumber(content, ["cpuCount"], 0);
  if (limitFps > 0) params.push(quoteParam("limitFPS", limitFps));
  if (cpuCount > 0) params.push(quoteParam("cpuCount", cpuCount));
  ...
  if (profileSetting) hints.push(`profilesPath imported from manager config: ${profileSetting}`);
  if (manualParams.length) hints.push(`${manualParams.length} manual serverLaunchParams entries were merged when usable.`);
...
function buildScriptLaunchSource(filePath: string, rootPath: string, detectedProfilePath: string, content: string): LaunchProfileSource | null {
  ...
  if (usesPsManagerLists) hints.push("PowerShell manager reads modServerPar.txt and serverModServerPar.txt for -mod/-serverMod.");
  if (hasDefaultPsLaunch) hints.push("PowerShell default launch pattern detected: config, mod, serverMod, bepath, profiles, port, freezecheck, adminlog, dologs.");
  ...
  if (!launchParams && !hints.length) return null;
  return {
    filePath,
    type: "script",
...
async function detectModFolders(rootPath: string) {
  try {
    const entries: Array<{ isDirectory(): boolean; name: string }> = await fs.readdir(rootPath, { withFileTypes: true });
...
async function findCandidateFiles(rootPath: string) {
  ...
  for (const dir of searchDirs) {
    try {
      const entries: Array<{ isFile(): boolean; name: string }> = await fs.readdir(dir, { withFileTypes: true });
      for (const entry of entries) {
        if (!entry.isFile()) continue;
        if (MANAGER_FILE_RE.test(entry.name) || SCRIPT_FILE_RE.test(entry.name)) files.push(path.join(dir, entry.name));
      }
    } catch {
...
async function readParamListSource(rootPath: string, detectedProfilePath: string): Promise<LaunchProfileSource | null> {
  const parentPath = path.dirname(rootPath);
  const candidates = [
  ...
  for (const [modsFile, serverModsFile] of candidates) {
    ...
    if (!mods.length && !serverMods.length) continue;
    const params = [
      quoteParam("config", path.join(rootPath, "serverDZ.cfg")),
...
async function readSourceFromFile(filePath: string, rootPath: string, detectedProfilePath: string): Promise<LaunchProfileSource | null> {
...
async function profileHasExpansion(profilePath: string) {
  if (!profilePath || !(await isDir(profilePath))) return false;
  return exists(path.join(profilePath, "ExpansionMod"));
}

function idsInLaunchParam(params: string, name: "mod" | "servermod") {
...
export async function detectLaunchProfile(input: { rootPath: string; profilePath?: string; launchParams?: string; }): Promise<LaunchProfileDetection> {
  ...
  const sources: LaunchProfileSource[] = [];

  const paramListSource = await readParamListSource(rootPath, profilePath);
  if (paramListSource) sources.push(paramListSource);
  ...
  sources.sort((a, b) => {
    ...
    const modScore = (source: LaunchProfileSource) => (source.modCount ?? 0) * 10 + (source.serverModCount ?? 0) * 5 + (hasParam(source.launchParams, "mod") ? 100 : 0);
  ...
  if (hasModFolders && !hasModParam) warnings.push(`${modFolders.length + modIdFolders.length} mod folders/IDs detected but launchParams has no -mod= entry. Import the old launch profile before starting a modded server.`);
  if (expansionDetected && !hasModParam) errors.push("Expansion indicators detected, but launchParams has no -mod= entry. Starting without the Expansion mod list can make the server exit during load.");
  if (expansionDetected && !hasServerModParam) warnings.push("Expansion profile/mod folders detected but no -servermod= entry found. This may be OK, but compare with the old manager profile.");
  if (!source && !explicitLaunchParams && hasModFolders) warnings.push("No old manager/start script launch profile was found automatically. Paste launchParams manually from your old manager.");
  if (!hasBepathParam && await exists(path.join(rootPath, "battleye"))) warnings.push("BattlEye folder detected but no -bepath= entry found. Add it if your old manager used a custom BattlEye path.");
