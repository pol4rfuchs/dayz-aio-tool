...
function clientId(request: FastifyRequest) {
...
function isPublicRoute(request: FastifyRequest) {
  ...
  if (request.url === "/favicon.ico") return true;
  if (request.url.startsWith("/api/auth/status")) return true;
  return false;
}
...
export function registerSecurityHooks(app: FastifyInstance) {
  ...
  app.addHook("onRequest", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (effectiveAuthDisabled() || isPublicRoute(request)) return;
