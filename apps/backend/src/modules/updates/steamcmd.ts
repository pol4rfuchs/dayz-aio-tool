...
export type SteamCmdQueueState = {
  running: boolean;
  runningLabel?: string;
  queued: number;
...
export type SteamCmdOutputAnalysis = {
  findings: string[];
  hardFailure: boolean;
  hasSuccess: boolean;
};

let steamCmdChain: Promise<void> = Promise.resolve();
...
export function parseSteamCmdOutput(output: string): SteamCmdOutputAnalysis {
...
export function steamFailureReason(exitCode: number, analysis: SteamCmdOutputAnalysis) {
...
export async function runSteamCmd(file: string, args: string[], options: { timeoutMs?: number; label?: string } = {}): Promise<SteamCmdResult> {
  ...
  steamCmdChain = previous.then(() => new Promise<void>((resolve) => { release = resolve; }));
