...
async function isWritableDirectory(target: string) {
  try {
...
function check(key: string, label: string, ok: boolean, message: string, pathValue?: string, warn = false, details?: unknown): DoctorCheck {
...
export async function doctorRoutes(app: FastifyInstance) {
  app.get("/api/system/doctor", async () => {
    ...
    try {
      const row = getDb().prepare("SELECT COUNT(*) as count FROM servers").get() as { count: number };
      checks.push(check("db.query", "SQLite query", true, `Database query works. Servers registered: ${row.count}.`));
    } catch (error) {
      checks.push(check("db.query", "SQLite query", false, (error as Error).message));
    }
  ...
  app.get("/api/servers/:serverId/doctor", async (request, reply) => {
    try {
      ...
      if (server.missionPath) checks.push(check("types.info", "types.xml file info", true, "File stat result attached.", path.join(server.missionPath, "db", "types.xml"), false, await fileInfo(path.join(server.missionPath, "db", "types.xml"))));
