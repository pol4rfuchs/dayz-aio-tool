...
function classifyCrashText(text: string): CrashClassification {
  const evidence: string[] = [];
  const actions: string[] = [];
  const entities: string[] = [];
  let category = "no-critical-pattern";
  let severity: CrashClassification["severity"] = "ok";
  let confidence: CrashClassification["confidence"] = "low";

  for (const match of text.matchAll(/corrupted upon "([^"]+)"/gi)) entities.push(match[1]);
  for (const match of text.matchAll(/Class:\s+'([^']+)'/gi)) entities.push(match[1]);

  if (/0x00020013|Client Build:\s*\d+\s*Server Build:\s*\d+/i.test(text)) {
    category = "client-server-build-mismatch"; severity = "fail"; confidence = "high";
  ...
  if (/Weapon\.OnStoreLoad|EntityAI::OnStoreLoad|corrupted scripted variables|trying to set muzzle index|Access violation/i.test(text) && /OnStoreLoad/i.test(text)) {
    category = "persistence-corruption"; severity = "fail"; confidence = "high";
  ...
  if (/signature check failed|wrong signature|not accepted by this server|\.bikey/i.test(text)) {
    category = "signature-or-key-mismatch"; severity = "fail"; confidence = "high";
  ...
  if (/missing addon|requires addon|cannot open object|cannot open file|No entry.*CfgPatches|Can't compile/i.test(text)) {
    category = category === "no-critical-pattern" ? "mod-dependency-or-pbo-error" : category; severity = severity === "ok" ? "fail" : severity; confidence = confidence === "low" ? "medium" : confidence;
  ...
  if (/access violation|unhandled exception|fatal/i.test(text) && severity === "ok") {
    category = "native-crash"; severity = "fail"; confidence = "medium";
...
function mergeClassifications(items: CrashClassification[]): CrashClassification {
  const ranking = { ok: 0, warn: 1, fail: 2 } as const;
  const bySeverity = [...items].sort((a, b) => ranking[b.severity] - ranking[a.severity]);
  const top = bySeverity[0] ?? { severity: "ok", category: "no-critical-pattern", confidence: "low", evidence: [], recommendedActions: [], entities: [] } as CrashClassification;
  return {
...
async function safeRealPath(input: string) {
  try { return await fs.realpath(input); }
  catch { return path.resolve(input); }
}

export async function walkCrashFiles(root: string, maxFiles = 750) {
  const files: Array<{ path: string; name: string; size: number; modifiedAt: string }> = [];
  const rootReal = await safeRealPath(root);

  async function isSafePath(candidate: string) {
    const real = await safeRealPath(candidate);
    try { assertInsideRoot(rootReal, real); return real; }
    catch { return null; }
  }
  ...
  async function visit(current: string) {
    ...
    let entries: Dirent[] = [];
    try { entries = await fs.readdir(currentReal, { withFileTypes: true }); } catch { return; }

    for (const entry of entries) {
      if (files.length >= maxFiles) return;
      const candidate = path.join(currentReal, entry.name);
      let stat;
      try { stat = await fs.lstat(candidate); } catch { continue; }
      if (stat.isSymbolicLink()) continue;
      const safeCandidate = await isSafePath(candidate);
      if (!safeCandidate) continue;

      if (stat.isDirectory()) {
        if (/node_modules|steamapps|@/i.test(entry.name)) continue;
...
async function tail(filePath: string, maxBytes = 12000) {
  const stat = await fs.lstat(filePath);
  if (stat.isSymbolicLink()) throw Object.assign(new Error("Refusing to read symlinked crash/log file."), { statusCode: 400 });
...
export async function crashRoutes(app: FastifyInstance) {
  app.get("/api/servers/:serverId/crash/scan", async (request, reply) => {
    try {
      ...
      const classifications: CrashClassification[] = [];
      if (runtimeHits.length) classifications.push(classifyCrashText(runtimeHits.join("\n")));
      for (const file of files.slice(0, 8)) {
        if (/\.mdmp$/i.test(file.name)) continue;
        try { classifications.push(classifyCrashText(await tail(file.path, 20000))); } catch { /* skip unreadable */ }
      }
  ...
  app.post("/api/servers/:serverId/crash/classify", async (request, reply) => {
    try {
      const text = String((request.body as any)?.text ?? "");
