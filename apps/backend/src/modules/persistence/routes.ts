...
async function existsDir(dir: string) {
  try { return (await fs.stat(dir)).isDirectory(); } catch { return false; }
}

async function dirSize(root: string, limitFiles = 20000) {
  let files = 0;
  let bytes = 0;
  async function walk(current: string) {
    if (files >= limitFiles) return;
...
function missionRoot(server: any) {
  if (!server.missionPath) throw Object.assign(new Error("Mission path is not configured."), { statusCode: 400 });
  return assertInsideRoot(server.rootPath, server.missionPath);
}

async function listQuarantines(missionPath: string) {
  let entries: import("node:fs").Dirent[] = [];
  try { entries = await fs.readdir(missionPath, { withFileTypes: true }); } catch { return []; }
  const result = [];
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    if (!/^storage_\d+_(?:DISABLED|QUARANTINE)_/i.test(entry.name)) continue;
...
export async function persistenceRoutes(app: FastifyInstance) {
  app.get("/api/servers/:serverId/persistence/scan", async (request, reply) => {
    try {
      ...
      const candidates = [];
      let entries: import("node:fs").Dirent[] = [];
      try { entries = await fs.readdir(missionPath, { withFileTypes: true }); } catch { entries = []; }
      for (const entry of entries) {
        if (!entry.isDirectory()) continue;
        if (!/^storage_\d+$/i.test(entry.name)) continue;
        const full = path.join(missionPath, entry.name);
        const stat = await fs.stat(full);
  ...
  app.post("/api/servers/:serverId/persistence/quarantine", async (request, reply) => {
    try {
      ...
      const server = requireServer(serverId);
      const status = getRuntimeStatus(serverId);
      if (status.pidAlive) return reply.code(400).send({ error: "Stop the DayZ server before quarantining persistence storage." });
      const missionPath = missionRoot(server);
      const source = assertInsideRoot(server.rootPath, path.join(missionPath, input.storageName));
      if (!await existsDir(source)) return reply.code(404).send({ error: `Storage folder not found: ${source}` });
  ...
  app.post("/api/servers/:serverId/persistence/restore", async (request, reply) => {
    try {
      ...
      const server = requireServer(serverId);
      const status = getRuntimeStatus(serverId);
      if (status.pidAlive) return reply.code(400).send({ error: "Stop the DayZ server before restoring persistence storage." });
      const missionPath = missionRoot(server);
      const quarantinePath = assertInsideRoot(server.rootPath, input.quarantinePath);
      if (!await existsDir(quarantinePath)) return reply.code(404).send({ error: `Quarantine folder not found: ${quarantinePath}` });
      ...
      if (await existsDir(active)) return reply.code(409).send({ error: `Active storage folder already exists: ${active}. Quarantine it first.` });
